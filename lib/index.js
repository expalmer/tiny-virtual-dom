...
function applyProps($el, newProps, oldProps = {}) {
  ...
  Object.keys(props).map(name => {
    ...
    if (isObject(newValue)) {
      ...
      const oValue = oldValue || {};
      const subProps = merge(nValue, oValue);
      Object.keys(subProps).map(subName => {
        if (!nValue[subName]) {
          removeProp($el[name], subName);
        } else if (nValue[subName] !== oValue[subName]) {
          setProp($el[name], subName, nValue[subName]);
        }
      });
    } else {
      if (!newValue) {
